...
for (i in 1:iter) {
  ...
  modorm<- update(modorm1, .~.
                  ...
                  + (rcs(age,3) + rcs(weight,3) + rcs(height,3) + rcs(oactual.24,3) + rcs(flex.0,3)  + rcs(ext.0,3) + rcs(sfpf.0,3) )   )   ## orm with splines
  ...
  modrf <-   train(dx , dy , 
    ...
    ## Using optimal tune parameters from previous (caret) anlayses
   myval <- modxgbm$bestTune
    ...
    learners1 <- create.Learner("SL.glmnet", detailed_names=T, tune=list(alpha = 0) )   ## Ridge regression L2 norm 
    ...
    for (j in names (modlist)) {
      ...
      ## Harrell's val.prob.R generate AUC, eavg (MAE), and Brier score
